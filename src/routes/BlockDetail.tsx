...
export const BlockDetail: React.FC = () => {
  ...
  const { preferences, addCorrection } = useReviewStore();
  ...
  const fetchDeepBlock = async () => {
    ...
    try {
      ...
      const hourNum = (hourId || '1').replace(/[^0-9]/g, '');
      const manifestUrl = `${import.meta.env.BASE_URL}assets/manifests/h${hourNum}_block_${blockId}.json`;
  ...
  const handleRegenerate = async (slide?: VisualAtom) => {
    ...
    const target = slide ? `slide_${slide.atom_id.split('-').pop()}` : 'all';
      ...
      <div className="flex-1 flex flex-col h-full bg-background-light dark:bg-background-dark overflow-hidden">
                        ...
                        <div key={safeId} className="border border-slate-200 dark:border-slate-800 rounded-lg overflow-hidden hover:border-primary/50 transition-colors">
                          ...
                          <div className="p-4 border-t border-slate-200 dark:border-slate-800 space-y-3">
                            <div className="flex items-center justify-between">
                              ...
                              {feedback && (
                                <span className={`text-[10px] px-2 py-1 rounded font-bold ${feedback.status === 'keep' ? 'bg-emerald-100 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-300' :
                ...
                <div className="space-y-4">
                  {quiz.map((q: any, idx) => {
                    ...
                    const getQuizContent = (quizAtom: any): string => {
                      try {
                        const content = quizAtom.content;
                        if (typeof content === 'string') return content;
                        if (!content) return 'No content';

                        // Handle SCORPION wrapped objects
                        if (content.question) return content.question;
                        if (content.text) return content.text;

                        // Handle generic objects (like the ones causing [object Object])
                        if (typeof content === 'object') {
                          // If it's a simple key-value pair, join them
        ...
        {showAtomEditor && selectedAtom && (
          <AtomEditorModal
            ...
            onSave={(feedback) => {
              handleAtomAction(selectedAtom.atom_id, feedback.status, feedback.notes);
              setShowAtomEditor(false);
            }}
          />
        )}
