...
export const BlockGrid: React.FC = () => {
    ...
    const { blocks, setBlocks, preferences, corrections } = useReviewStore();
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState<string | null>(null);
    const [currentPage, setCurrentPage] = useState(1);

    const handleExportHourJSON = () => {
        ...
        downloadFile(json, `corrections-h${hourId}-${new Date().toISOString().split('T')[0]}.json`, 'application/json');
    };

    const handleExportCSV = () => {
        const hourCorrections = corrections.filter(c => c.hourId === hourId);
        if (hourCorrections.length === 0) {
            alert("No corrections to export for this hour.");
            return;
        }

        const headers = ["Block ID", "Asset Type", "Asset Name", "Issue", "Priority", "Status", "Timestamp"];
        const rows = hourCorrections.map(c => [
            c.blockId,
            c.assetType,
            c.assetName,
            `"${c.issue.replace(/"/g, '""')}"`,
        ...
        downloadFile(csvContent, `corrections-h${hourId}-${new Date().toISOString().split('T')[0]}.csv`, 'text/csv');
    };
        ...
        <ErrorBoundary>
            ...
            <header className="h-16 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-[#111318] flex items-center justify-between px-8 shrink-0">
                ...
                <div className="flex items-center gap-2">
                    <button onClick={handleExportCSV} className="flex items-center gap-2 h-9 px-4 bg-emerald-600 text-white text-xs font-bold rounded-lg hover:bg-emerald-700 transition-colors">
                        <span className="material-symbols-outlined text-lg">csv</span>
                        CSV
                    </button>
                    <button onClick={handleExportHourJSON} className="flex items-center gap-2 h-9 px-4 bg-primary text-white text-xs font-bold rounded-lg hover:bg-blue-700 transition-colors">
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar">
                ...
                {blocks.length > 0 && blocks[0].images.length > 0 && (
                    <div className="px-8 pt-8 shrink-0">
                        <div className="relative group aspect-[21/9] w-full bg-slate-100 dark:bg-[#1c1f27] rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-800 shadow-sm">
                            <img
                                src={blocks[0].images[0]}
                            ...
                            <div className="absolute inset-0 bg-gradient-to-t from-slate-900/80 via-slate-900/20 to-transparent flex flex-col justify-end p-8">
                                <div className="max-w-2xl">
                                    <div className="flex items-center gap-2 mb-3">
                                        <span className="px-2 py-0.5 bg-primary/20 text-primary-light text-[10px] font-bold uppercase tracking-wider rounded border border-primary/30">First Block Preview</span>
                                        <span className="text-slate-400 text-xs">{blocks[0].title}</span>
                                    </div>
                                    <h1 className="text-3xl font-black text-white mb-2 tracking-tight">Hour {hourId.padStart(2, '0')}: {blocks[0].hourId}</h1>
                                    <p className="text-slate-300 text-sm max-w-lg line-clamp-2">
                                        Starting review for {blocks.length} content blocks. This visual anchor represents the opening scenario of the lesson.
                                    </p>
                                </div>
                            </div>
                        </div>
